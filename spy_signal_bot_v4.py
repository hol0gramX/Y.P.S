...
def trend_failed_early_exit(row, prev):
    return (
        (row['Close'] < prev['Close'] * 0.996) and
        (row['RSI'] < 50) and
        (row['Close'] < row['VWAP'])
    )
...
def generate_signal(df):
    ...
    if current_pos == "call":
        if trend_failed_early_exit(row, prev):
            state["position"] = "none"
            save_last_signal(state)
            return time_index, "🚫 主升浪方向失败，提前止损退出"
        elif check_call_exit(row):
            state["position"] = "none"
            save_last_signal(state)
        ...
        if trend_failed_early_exit(row, prev):
            state["position"] = "none"
            save_last_signal(state)
            return time_index, "🚫 主跌浪方向失败，提前止损退出"
        elif check_put_exit(row):
            state["position"] = "none"
            save_last_signal(state)
