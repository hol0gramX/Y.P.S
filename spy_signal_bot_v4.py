...
def check_call_entry(row):
    return row['Close'] > row['VWAP'] and row['RSI'] > 50 and row['MACDh'] > -0.1 and strong_volume(row)

def check_put_entry(row):
    return row['Close'] < row['VWAP'] and row['RSI'] < 51 and row['MACDh'] < 0.15 and strong_volume(row)
...
def allow_call_reentry(row, prev_row):
    return (
...
def main():
    try:
        ...
        if not is_market_open_now():
            print(f"[{get_est_now().strftime('%Y-%m-%d %H:%M:%S %Z')}] 🕒 盘前/盘后，跳过信号判断")
            return
        df = get_data()
        ...
        if signal:
            ...
            print(f"[{get_est_now().strftime('%Y-%m-%d %H:%M:%S %Z')}] 无交易信号")
